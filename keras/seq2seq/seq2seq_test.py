# -*- encoding:utf-8 -*-
'''
seq2seq from : https://github.com/farizrahman4u/seq2seq
...
def main():
    ...
    def shuffle_li(li1,li2):
      indexs = range(len(li1))
      random.shuffle(indexs)
      a = [li1[i] for i in indexs]
      b = [li2[i] for i in indexs]
      return a,b

    for i in range(10):
      cur_len = len(input_list)
      for j in range(len(input_list)):
        a,b = shuffle_li(input_list[j],tar_list[j])
        input_list.append(a)
        input_text.append(" ".join(a))
        tar_list.append(b) 
        tar_text.append(" ".join(b))
    ...
    #en_de_model.add(SimpleSeq2Seq(output_dim=output_dim, output_length=tar_maxlen,input_shape=(input_maxlen,hidden_dim)))
    en_de_model.add(AttentionSeq2Seq(output_dim=output_dim, output_length=tar_maxlen,input_shape=(input_maxlen,hidden_dim),dropout=0.5))
    
    print en_de_model.summary()
